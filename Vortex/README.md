This is a summary of all of FATALITY's commands. Arguments surrounded by `<>` are required, and arguments surrounded by `[]` are optional. Do not include `<>` nor `[]` when running commands!
...
### `??invite`
Provides FATALITY's invite link. If you are having trouble inviting FATALITY, make sure you are correctly logged in at https://discordapp.com/login

### `??ping`
Checks FATALITY's latency. 
...
### `??voicemove [channel]`
Puts FATALITY into the specified voice channel (or your current channel if no arguments are provided) and waits. When you drag FATALITY to a new channel, he will drag all users in the current channel to the new channel.
...
Edits a reason in the Moderation Log. If no case number is provided, FATALITY will search the log for the most-recent case that has no provided reason.


## 📜 Settings Commands
These commands control various FATALITY settings. The Manage Server permission is needed to use these commands.
...
Sets the channel to which avatar changes of members are logged. This feature is only available with [[FATALITY Pro]].
...
### `??resolvelinks <ON | OFF>`
Enables or disables resolving links (redirect links) for invite and referral links. This feature is only available with [[FATALITY Pro]].

### `??autoraidmode <ON | OFF | joins/seconds>`
Configures FATALITY to automatically enable Anti-Raid Mode when it senses a raid. Setting `joins/seconds` will enable Anti-Raid Mode if there are at least `joins` new joins to the server within `seconds` seconds. Setting this to `ON` uses 10 joins in 10 seconds.
...
### `??lookup <ID | invite>`
Finds information about a user or guild via ID (for either) or an invite code (for guilds only). All information found in this way is information that Discord makes publicly-available, and is not impacted by FATALITY being in the guilds nor having mutual servers with the user.
...
### Cleaning Messages
FATALITY's `??clean` command supports many options and filters. Check out the page on [[Cleaning Messages]] for a full overview of this feature.
